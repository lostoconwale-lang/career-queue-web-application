...
export default function HowItWorks({ content }: { content: PublicHowItWorksDTO }) {
  return (
    <section id="how-it-works" className="mx-auto max-w-6xl px-5 py-24 sm:px-8 sm:py-32">
      <Reveal className="text-center">
        <h2 className="font-display text-ink flex flex-wrap items-center justify-center gap-x-4 gap-y-1 text-4xl leading-[1.1] font-semibold tracking-tight sm:gap-x-7 sm:text-6xl">
          {content.steps.map((step, index) => (
            <span key={index} className="flex items-center gap-x-4 sm:gap-x-7">
              {index > 0 ? (
                <Arrow
                  className="text-coral h-6 w-6 shrink-0 sm:h-9 sm:w-9"
                  strokeWidth={2.2}
                />
              ) : null}
              <span>{step.word}</span>
        ...
        <p className="text-muted mx-auto mt-6 max-w-lg text-lg">{content.subtext}</p>
      </Reveal>
          ...
          <Reveal key={index} delay={index * 0.1}>
            <li className="rounded-card border-line bg-surface shadow-soft h-full border p-8">
              <div className="flex items-center justify-between">
                ...
                <img src={step.imageUrl} alt="" className="h-20 w-20 object-contain" />
              </div>
