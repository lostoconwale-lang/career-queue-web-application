...
import { setAdminPasswordFormSchema } from "@/lib/validators/admin.validator";
...
export function SetPasswordDialog({
  ...
  function validateField(key: FieldKey) {
    const result = setAdminPasswordFormSchema.safeParse(form);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const parsed = setAdminPasswordFormSchema.safeParse(form);
    if (!parsed.success) {
    ...
    try {
      const res = await fetch(`/api/v1/admin/${admin.id}/password`, {
        method: "PUT",
