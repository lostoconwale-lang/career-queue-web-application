...
import { createAdminFormSchema } from "@/lib/validators/admin.validator";
...
export function CreateAdminDialog({
  ...
  function validateField(key: FieldKey) {
    const result = createAdminFormSchema.safeParse(form);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const parsed = createAdminFormSchema.safeParse(form);
    if (!parsed.success) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-20">
      ...
      <div
        ...
        className="bg-surface shadow-lift relative grid w-full max-w-2xl overflow-hidden rounded-3xl md:grid-cols-[0.8fr_1fr]"
      >
        <div className="bg-brand-soft relative hidden md:block">
          ...
          <div className="absolute inset-x-0 bottom-0 p-6">
            <p className="text-surface font-display text-lg font-semibold">Add a teammate</p>
            <p className="text-surface/80 mt-1 text-xs">
              New admins can sign in immediately with the password you set.
            </p>
          </div>
        </div>

        <div className="p-6 sm:p-7">
          {/* <h2 id="create-admin-title" className="font-display text-ink text-xl font-semibold">
            New admin
          </h2> */}
          {/* <p className="text-muted mt-1 text-sm">Approved and active from the start.</p> */}
