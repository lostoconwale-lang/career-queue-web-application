...
export function CityList() {
  ...
  async function toggleActive(city: CityDTO) {
    setBusyId(city.id);
    setError(null);
    try {
      const res = await fetch(`/api/v1/cities/${city.id}`, {
        method: "PUT",
      ...
      setError("Could not update that city. Please try again.");
    } finally {
      setBusyId(null);
    }
  }
  ...
  async function confirmDelete() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/v1/cities/${deleting.id}`, { method: "DELETE" });
      if (redirectOnDenied(res)) return;
      if (!res.ok) {
        const json = (await res.json()) as ApiResponse<unknown>;
        setError(json.success ? "Could not delete that city." : json.error.message);
        return;
      }
      setReloadKey((k) => k + 1);
    } catch {
      setError("Could not delete that city. Please try again.");
    } finally {
      setBusyId(null);
      setDeleting(null);
    }
    ...
    },
    {
      ...
      cell: (c) => (
        <div className="flex justify-end gap-2">
          <ActionButton disabled={busyId === c.id} onClick={() => setEditing(c)}>
            Edit
          </ActionButton>
    ...
    <div className="px-5 py-10 sm:px-8">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted mt-2 text-sm">
            The cities available across the job board. Turn one off to hide it without deleting.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="mt-8 flex flex-wrap items-center gap-3">
        ...
        <SearchInput
          ...
          ariaLabel="Search cities by name"
      ...
      <div className="mt-5">
        <Table
          ...
          rowKey={(c) => c.id}
          loading={loading}
          emptyMessage="No cities match this view."
          minWidth={720}
        />
      </div>
      ...
      <ConfirmDialog
        ...
        description={
          deleting ? `"${deleting.name}" will be removed from the job board.` : null
        }
        confirmLabel="Delete"
