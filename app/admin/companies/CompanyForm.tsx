...
import { createCompanyBodySchema } from "@/lib/validators/company.validator";
...
export function CompanyForm({ companyId }: { companyId?: string }) {
  ...
  const [logo, setLogo] = useState<EmbeddedMediaDTO | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/v1/companies/${companyId}`, { cache: "no-store" })
      .then((res) => {
        ...
        return res.json() as Promise<ApiResponse<CompanyDTO>>;
      })
      .then((json) => {
        ...
        setLogo(c.logo);
        if (c.logo) setLogoName(c.logo.key.split("/").pop() ?? "");
      })
      .catch(() => {
        if (alive) setLoadError("Could not load this company.");
      })
      .finally(() => {
  ...
  function validate() {
    const result = createCompanyBodySchema.safeParse({ name, website });
    const message = result.success
      ? undefined
      : (result.error.flatten().fieldErrors.name?.[0] ??
        result.error.flatten().fieldErrors.website?.[0]);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const body = {
      ...
      logo: logo ? { key: logo.key } : null,
    };

    setSubmitting(true);
    try {
      const res = await fetch(editing ? `/api/v1/companies/${companyId}` : "/api/v1/companies", {
      ...
      if (!json.success) {
        ...
        else if (details.website?.length) setFieldError(details.website[0]);
  ...
  return (
    <div className="px-5 py-10 sm:px-8">
      <Link href="/admin/companies" className="text-muted hover:text-ink text-sm font-medium">
        ← Companies
      </Link>
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-6" noValidate>
        <section className="bg-surface border-line shadow-soft mx-auto max-w-2xl rounded-2xl border p-6 sm:p-7">
          ...
          <div className="mt-6 space-y-5">
            ...
            <div>
              <span className="text-ink text-sm font-medium">Logo</span>
              <p className="text-muted mt-0.5 text-xs">
                A small square image shown next to the company. Best around 128×128.
              </p>
              ...
              {logo ? (
                <div className="border-line mt-2 flex items-center gap-4 rounded-2xl border p-3">
                  ...
                  >
                    ...
                    <img src={logo.url} alt="" className="h-full w-full object-contain p-1.5" />
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-ink truncate text-sm font-medium" title={logoName}>
                      {logoName || "Selected logo"}
                    </p>
                    <p className="text-muted mt-0.5 text-xs">
                ...
                >
                  ...
                  <span className="text-ink text-sm font-semibold">Choose a logo</span>
      ...
      {picking ? (
        <MediaPicker
          ...
          onPick={(media) => {
            setLogo({ id: media.id, key: media.key, url: media.url });
            setLogoName(media.originalName);
            setPicking(false);
          }}
        />
      ) : null}
