...
export function CompanyList() {
  ...
  async function toggleActive(company: CompanyDTO) {
    ...
    try {
      ...
      setError("Could not update that company. Please try again.");
    } finally {
      setBusyId(null);
    }
  }
  ...
  async function confirmDelete() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/v1/companies/${deleting.id}`, { method: "DELETE" });
      if (redirectOnDenied(res)) return;
      if (!res.ok) {
        const json = (await res.json()) as ApiResponse<unknown>;
        setError(json.success ? "Could not delete that company." : json.error.message);
        return;
      }
      setReloadKey((k) => k + 1);
    } catch {
      setError("Could not delete that company. Please try again.");
    } finally {
      setBusyId(null);
      setDeleting(null);
    }
    ...
    },
    {
      ...
      cell: (c) => (
        <div className="min-w-0">
          ...
          {c.website ? (
            <p className="text-muted mt-0.5 truncate text-xs">{c.website}</p>
          ) : null}
        </div>
      ),
    },
    ...
    <div className="px-5 py-10 sm:px-8">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted mt-2 text-sm">
            The companies jobs can be posted under. Turn one off to hide it without deleting.
          </p>
        </div>
        <Link
      ...
      <div className="mt-5">
        <Table
          ...
          rowKey={(c) => c.id}
          loading={loading}
          emptyMessage="No companies match this view."
          minWidth={720}
        />
      </div>
