...
export function TestimonialList() {
  ...
  const [data, setData] = useState<CursorPage<TestimonialDTO> | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [busyId, setBusyId] = useState<string | null>(null);
  const [reloadKey, setReloadKey] = useState(0);

  const [deleting, setDeleting] = useState<TestimonialDTO | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/v1/testimonials?${params.toString()}`, { cache: "no-store" })
      .then((res) => {
        if (redirectOnDenied(res)) return null;
        return res.json() as Promise<ApiResponse<CursorPage<TestimonialDTO>>>;
      })
      .then((json) => {
      ...
      .catch(() => {
        if (alive) setError("Could not load testimonials.");
      })
      .finally(() => {
  ...
  async function toggleActive(t: TestimonialDTO) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/v1/testimonials/${t.id}`, {
        method: "PUT",
      ...
      setError("Could not update that testimonial. Please try again.");
    } finally {
      setBusyId(null);
    }
  }
  ...
  async function confirmDelete() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/v1/testimonials/${deleting.id}`, { method: "DELETE" });
      if (redirectOnDenied(res)) return;
      if (!res.ok) {
        const json = (await res.json()) as ApiResponse<unknown>;
        setError(json.success ? "Could not delete that testimonial." : json.error.message);
        return;
      }
      setReloadKey((k) => k + 1);
    } catch {
      setError("Could not delete that testimonial. Please try again.");
    } finally {
      setBusyId(null);
      setDeleting(null);
    }
  ...
  const columns: Column<TestimonialDTO>[] = [
    {
      ...
      cell: (t) => (
        ...
        <img
          src={t.image.url}
          alt=""
          className="border-line bg-cream h-9 w-9 rounded-full border object-cover"
        />
      ),
    },
    {
      ...
      cell: (t) => (
        <div className="min-w-0">
          <p className="text-ink font-semibold">{t.authorName}</p>
          <p className="text-muted mt-0.5 truncate text-xs">{t.role}</p>
        </div>
      ),
    },
    {
      key: "rating",
      header: "Rating",
      cell: (t) => <Stars value={t.rating} />,
    },
    {
      key: "quote",
      header: "Testimonial",
      cell: (t) => <p className="text-muted line-clamp-2 max-w-md text-sm">{t.quote}</p>,
    },
    {
      key: "isActive",
      header: "Active",
    ...
    },
    {
      ...
      header: "Created",
      cell: (t) => (
        <span className="text-muted whitespace-nowrap" title={formatDateTime(t.createdAt)}>
    ...
    <div className="px-5 py-10 sm:px-8">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted mt-2 text-sm">
            Customer quotes shown across the site. Turn one off to hide it without deleting.
          </p>
        </div>
        <Link
      ...
      <div className="mt-8 flex flex-wrap items-center gap-3">
        ...
        <SearchInput
          ...
          ariaLabel="Search testimonials by name, role or quote"
      ...
      <div className="mt-5">
        <Table
          ...
          rowKey={(t) => t.id}
          loading={loading}
          emptyMessage="No testimonials match this view."
          minWidth={860}
        />
      </div>
      ...
      <ConfirmDialog
        ...
        description={
          deleting ? `The testimonial from "${deleting.authorName}" will be removed.` : null
        }
        confirmLabel="Delete"
...
function Stars({ value }: { value: number }) {
  return (
    <span className="inline-flex items-center gap-0.5" title={`${value} out of 5`}>
      {[1, 2, 3, 4, 5].map((n) => (
        <Star key={n} className={`h-3.5 w-3.5 ${n <= value ? "text-amber-400" : "text-line"}`} />
      ))}
    </span>
  );
}
